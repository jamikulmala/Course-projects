...
Student Id: 150425043
Name:       Jami Kulmala
Email:      jami.kulmala@tuni.fi

Program that models the actions that can be done to moving boxes.
"""
...
class MovingBox:
    ...
    def __init__(self, box_name):
        """
        Object moving box has a name and its contents.
        First the box is empty, but it is possible to fill it.
        Item name as dicts key and item count as its value.
    ...
    def add_item(self, item_name, item_count):
        """
        Adds the item and count to the dict.
        :param item_name: str, item to be added to the box, for example a plate
        :param item_count: int, amount of the item to be added from the box
        :return: None
        """

        # If the item is not in the box,
        # create a new key and set count as its value.
    ...
    def remove_item(self, item_name, item_count):
        """
        Removes selected amount of items from the box.
        :param item_name: str, item to be removed from the box
        :param item_count: int, amount of the item to be removed from the box
        :return: True if removing succeeded. False if something fails
        """

        # Check if the item to be removed is in the box
        # After that check that the box contains enough items.
        # After success, remove items from box.
        if item_name in self.__contents:
            if item_count <= self.__contents[item_name]:
                self.__contents[item_name] -= int(item_count)
                return True

        # If either of the conditions doesn't fill, the method returns False.
        else:
            return False

    def list_content(self):
        """
        Prints the information of a selected box
        :return: None
        """
        # For easier use, lets call self.__contents "box".
        box = self.__contents

        # Iterate through box and get its item count.
        count = 0

        for j in sorted(box):
            count += int(box[j])

        print(f"Box \"{self.__name}\" contains {count} items.")

        for i in sorted(box):
            # item counts are evened to right.
            # After this item names are also in the same column.
            # if count between 1-9 two spaces are added.
            # if count between 10-99 one space is added.
            # if count 100 and above no space is added.
            # contents are printed in alphabetical order.

            if box[i] in range(1, 10):
                print(f"  {box[i]} {i}")

            elif box[i] in range(10, 100):
                print(f" {box[i]} {i}")

            elif box[i] >= 100:
                print(f"{box[i]} {i}")

            # if item count is 0 or below, delete the item from the box
            else:
                self.__contents.pop(i)

    def transfer_item(self, target, item_name, item_count):
        """
        Moves selected amount of items from one box to another.
        :param target: MovingBox, box to which the items are transferred
        :param item_name: str, the item to be transferred
        :param item_count: int, amount of the item to be transferred
        :return: True if transferring succeeded. False if something failed.
        """
        # Check whether the item is in the box
        if item_name in self.__contents:
            # if the amount to be transferred is equal or lower than the amount
            # of item in the box, transferring can be committed.

            if item_count <= self.__contents[item_name]:
                self.__contents[item_name] -= item_count

                # The box to which the items are moved can have an amount of
                # the item of its own or not. If the item's already in box,
                # Increase its count. Elsewise create a new item to box

                if item_name in target.__contents:
                    target.__contents[item_name] += item_count
                    return True

                elif item_name not in target.__contents:
                    target.__contents[item_name] = item_count
                    return True
    ...
    def search_item(self, item_name):
        """
        Checks whether the given item is in the movingbox or no.
        :param item_name: item to be searched for
        :return: 0 if item not found, int, item count if item found
        """
        if item_name not in self.__contents:
            return 0
        else:
